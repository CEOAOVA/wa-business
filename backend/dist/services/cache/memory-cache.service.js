...
exports.messageCache = exports.userCache = exports.conversationCache = exports.MemoryCache = void 0;
...
class MemoryCache extends events_1.EventEmitter {
    constructor(options = {}) {
        ...
        this.startCleanup();
        logger_1.logger.info(`✅ Cache inicializado: ${this.options.evictionPolicy} con ${this.options.maxEntries} entradas máx.`);
    }
    /**
    ...
    set(key, value, ttl) {
        try {
            ...
            logger_1.logger.error('Error estableciendo valor en caché', { key, error });
        }
    }
    /**
     ...
     */
    shouldEvict(newSize) {
        return (this.stats.entries >= this.options.maxEntries ||
    ...
    evict(requiredSize) {
        ...
        for (const key of entriesToEvict) {
            ...
            logger_1.logger.debug('Cache eviction', { key, policy: this.options.evictionPolicy });
        }
        this.emit('eviction', {
     ...
     */
    getEvictionCandidates(requiredSize) {
        const candidates = [];
        let freedSize = 0;
    ...
    startCleanup() {
        this.cleanupInterval = setInterval(() => {
            ...
            if (cleaned > 0) {
                logger_1.logger.debug(`✅ Cache cleanup: ${cleaned} entradas expiradas eliminadas`);
    ...
    clear() {
        ...
        logger_1.logger.info(`✅ Cache limpiado: ${entries} entradas eliminadas`);
    ...
    destroy() {
        ...
        logger_1.logger.info('✅ Cache destruido');
    }
}
exports.MemoryCache = MemoryCache;
// ✅ Instancias singleton para diferentes propósitos
exports.conversationCache = new MemoryCache({
    maxEntries: 500,
...
exports.messageCache = new MemoryCache({
    maxEntries: 2000,
