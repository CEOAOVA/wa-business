...
exports.csvInventoryService = exports.CsvInventoryService = void 0;
/**
...
class CsvInventoryService {
    ...
    searchByNombre(text) {
        ...
        const normalizedText = (0, text_processing_1.normalizeForSearch)(text);
        ...
        const results = this.inventoryData.filter(row => {
            const nombre = (0, text_processing_1.normalizeForSearch)(row.Nombre || row.nombre || '');
            const marca = (0, text_processing_1.normalizeForSearch)(row.Marca || row.marca || '');
            const categoria = (0, text_processing_1.normalizeForSearch)(row.Categoria || row.categoria || '');
            const descripcion = (0, text_processing_1.normalizeForSearch)(row.Descripcion || row.descripcion || '');
            const fullText = `${nombre} ${marca} ${categoria} ${descripcion}`.toLowerCase();
            // Verificar si todos los tokens están presentes
            return searchTokens.every(token => fullText.includes(token.toLowerCase()));
        });
        console.log(`[CsvInventoryService] Encontrados ${results.length} resultados`);
        // Mostrar primeros resultados para debug
        if (results.length > 0) {
            console.log(`[CsvInventoryService] Primeros resultados:`, results.slice(0, 3).map(r => `${r.Clave || r.codigo} - ${r.Nombre || r.nombre}`));
        }
        return results;
    }
    /**
     ...
     */
    searchByCodigo(codigo) {
        if (!codigo || !this.isLoaded)
            return null;
    ...
    searchByCategoria(categoria) {
        if (!categoria || !this.isLoaded)
            return [];
        const normalizedCategoria = (0, text_processing_1.normalizeForSearch)(categoria);
        return this.inventoryData.filter(row => {
            const rowCategoria = (0, text_processing_1.normalizeForSearch)(row.Categoria || row.categoria || '');
     ...
     */
    searchByMarca(marca) {
        if (!marca || !this.isLoaded)
            return [];
        const normalizedMarca = (0, text_processing_1.normalizeForSearch)(marca);
        return this.inventoryData.filter(row => {
            const rowMarca = (0, text_processing_1.normalizeForSearch)(row.Marca || row.marca || '');
...
exports.CsvInventoryService = CsvInventoryService;
// Exportar instancia singleton
exports.csvInventoryService = new CsvInventoryService();
