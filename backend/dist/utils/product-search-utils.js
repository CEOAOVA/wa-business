...
exports.extractCarDataFromMessage = extractCarDataFromMessage;
exports.formatSearchResults = formatSearchResults;
exports.parseUserConfirmation = parseUserConfirmation;
exports.generateSearchSuggestions = generateSearchSuggestions;
/**
 * Extraer datos del auto de un mensaje de texto
 */
function extractCarDataFromMessage(message) {
    const carData = {};
    // Extraer marca
 ...
 */
function formatSearchResults(matches, carData) {
    if (matches.length === 0) {
    ...
    matches.slice(0, 5).forEach((match, index) => {
        ...
        if (match.marca)
            response += `   🚗 Marca: ${match.marca}\n`;
        if (match.año)
            response += `   📅 Año: ${match.año}\n`;
        response += `\n`;
    });
    if (matches.length > 5) {
 ...
 */
function parseUserConfirmation(userResponse) {
 ...
 */
function generateSearchSuggestions(userTerm) {
    const suggestions = [
