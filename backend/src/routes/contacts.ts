...
import { SupabaseContact } from '../services/supabase-database.service';
...
router.get('/conversation/:phoneNumber', async (req: any, res: any) => {
  try {
    ...
    const contact = await supabaseDatabaseService.getContactByPhone(phoneNumber);
    ...
    const conversation = await supabaseDatabaseService.getOrCreateConversation(phoneNumber);
 ...
 */
router.put('/:contactId', async (req: any, res: any) => {
  try {
    ...
    const updateData: Partial<SupabaseContact> = {};
    ...
    const success = await supabaseDatabaseService.updateContactWithPostalCode(contactId, updateData);
...
router.post('/migrate', async (req: any, res: any) => {
  try {
    ...
    const result = await supabaseDatabaseService.addPostalCodeToContacts();
